...
function ContactForm({ prefix }: { prefix: string }) {
  return (
    <form className="space-y-4">
      <h3 className="font-heading text-2xl font-normal text-brand mb-6">Schreiben Sie uns</h3>
...
export default function KontaktPage() {
  return (
    <>
      ...
      {/* ── Variant A: Portrait-forward ── */}
      <section id="kontaktformular" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-16">
        <p className="text-[10px] uppercase tracking-[0.22em] text-accent mb-10">Variante A</p>
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_2fr] gap-12">
          <div className="min-w-0">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {team.map((person) => (
                <div key={person.name} className="group relative overflow-hidden" style={{ aspectRatio: '3/4' }}>
                  <Image
                    src={person.src}
                    alt={person.name}
                    fill
                  ...
                  />
                  <div className="absolute inset-x-0 bottom-0 h-2/5 bg-gradient-to-t from-brand-dark/90 to-transparent" />
                  <div className="absolute inset-x-0 bottom-0 p-4">
                    <p className="text-[9px] uppercase tracking-[0.18em] text-accent mb-1">{person.role}</p>
                    <p className="font-heading text-base font-normal text-warm-50 leading-snug">{person.name}</p>
                    {person.email && (
                      <a href={`mailto:${person.email}`} className="inline-block mt-1 text-[10px] text-warm-300 hover:text-warm-50 transition-colors">
                        {person.email}
                      </a>
                    )}
                  </div>
          ...
          <div>
            <ContactForm prefix="a" />
          </div>
        </div>
      </section>

      <div className="border-t border-warm-200 mx-8" />

      {/* ── Variant B: Horizontal card ── */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-16">
        <p className="text-[10px] uppercase tracking-[0.22em] text-accent mb-10">Variante B</p>
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_2fr] gap-12">
          <div className="min-w-0">
